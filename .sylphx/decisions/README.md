# Architecture Decision Records (ADRs)

Index of all architectural decisions for NeuronLine.

## Active

- [ADR-001](001-pure-functional-architecture.md) - Pure Functional Architecture
- [ADR-002](002-tiled-matrix-multiplication.md) - Tiled Matrix Multiplication
- [ADR-003](003-optional-acceleration-packages.md) - WASM/WebGPU as Optional Packages
- [ADR-004](004-memory-pooling.md) - Memory Pooling with Scope-Based Lifetime Management

## Proposed

None

## Superseded

None

## Format

Each ADR follows this structure:
- **Status**: ✅ Accepted | 🚧 Proposed | ❌ Rejected | 📦 Superseded
- **Context**: Problem statement
- **Decision**: What we decided
- **Rationale**: Why (bullet points)
- **Consequences**: Positive and negative outcomes
- **References**: Implementation files (with VERIFY markers)
